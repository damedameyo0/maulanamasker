...
class Pembelian_model extends CI_Model
{
    ...
    { 
        $query = $this->db->query("SELECT MAX(pembelian_id) as pembelian_id
            FROM pembelian"); return $query->row(); 
    }
    ...
    {
        $query=$this->db->query("SELECT pembelian.pembelian_id, pembelian.kode_beli, supplier.supplier_id, supplier.name as supplier_name, employee.employee_id,
            employee.name as employee_name, pembelian.buy_date FROM pembelian LEFT JOIN supplier ON pembelian.supplier_id=supplier.supplier_id INNER JOIN employee ON pembelian.employee_id=employee.employee_id");
        return $query;
    }
    ...
    public function rangeDate($start_date, $end_date) // menarik data dr tabel pembelian, supplier, dan employee sesuai rentang tanggal d ari inputan user
    {
     $query = $this->db->query("SELECT pembelian.pembelian_id, pembelian.kode_beli, supplier.supplier_id, supplier.name as supplier_name, employee.employee_id, employee.name as employee_name, pembelian.buy_date FROM pembelian
   ...
   public function monthChart($start_date, $end_date) // menghitung jml buku yang dibeli per bulan (current year)
   {
    $query = $this->db->query("SELECT pembelian.pembelian_id, d_pembelian.pembelian_id, SUM(d_pembelian.amount) as
        count, MONTHNAME(buy_date) as month_name, YEAR(CURDATE()) as year_num FROM pembelian
